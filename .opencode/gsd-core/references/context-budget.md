...
The keys live in `.claude/settings.json` (project) or `/Users/girdharisahu/Projects/TransportApp/.opencode/settings.json` (global) — **not** in `.planning/config.json`:
...
See @/Users/girdharisahu/Projects/TransportApp/.opencode/gsd-core/references/planner-guidance.md for Granularity Calibration table (Coarse/Standard/Fine plans-per-phase).
