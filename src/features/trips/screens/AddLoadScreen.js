...
export default function AddLoadScreen() {
  ...
  const {
    ...
    defaultValues: {
      partyName: '',
      truckNumber: parentTrip?.truckNumber || 'KA12DS3747',
      driverName: parentTrip?.driverName || '',
      origin: parentTrip?.destination || 'Hyderabad',
      destination: '',
      billingType: 'Fixed',
      freightAmount: '',
      ...
      material: '',
      startKm: '',
      note: parentTrip?.id ? `Additional load linked to parent trip ${parentTrip.id}` : '',
    },
  });
              ...
              <Controller
                ...
                render={({field: {value}}) => (
                  <TouchableOpacity
                    ...
                    <AppText variant="caption" color="textMuted" style={styles.subHint}>
                      Optional
                    </AppText>
                  </TouchableOpacity>
                )}
