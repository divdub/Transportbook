...
$pdf->SetFillColor(175, 213, 240);
  ...
  $pdf->SetFillColor(211, 211, 211);
	...
	$pdf->Cell(19,6,'GR DATE',1,0,'L',1);
	$pdf->Cell(24,6,'GR/TR NO',1,0,'L',1);
	$pdf->Cell(20,6,'DI NO ',1,0,'L',1);
	...
	$pdf->Cell(20,6,'DIESEL AMT',1,0,'C',1);
		$pdf->Cell(25,6,'PAID DATE',1,0,'C',1);
		$pdf->Cell(26,6,'PAID AMT',1,1,'C',1);
    ...
    $pump_name = $cmn->getvalfield($connection,"m_petrol_pump","pump_name","pump_id='$row[pump_id]'");
         $dbillno = $cmn->getvalfield($connection,"dieselbill","dbillno","dbillid='$row[dbillid]'");
                $is_pay = $cmn->getvalfield($connection,"dieselbill","is_pay","dbillid='$row[dbillid]'");
               if($is_pay!=0 )
               {
                   $total_amt1=$total_amt;
                     $rcv_date = date('d-m-Y',strtotime($cmn->getvalfield($connection,"diesel_pay","rcv_date","dbillid='$row[dbillid]'")));
               }
               else{
                   $total_amt1='0';
                  $rcv_date='';
...
$pdf->Cell(25,8,$rcv_date,1,0,'R');
       ...
       $pdf->SetX(2);
                                        ...
                                        $pdf->Line($currentX , $currentY + 16, $currentX + 293, $currentY + 16); 
                                                     $pdf->SetFont('Arial','B',9);
                                                     $pdf->Cell(222,12,'Total',1,0,'C','F');

                                                     
                                                  
                                                      $pdf->SetFont('Arial','B',8);
                                                      $pdf->Cell(20,12,$total_bal,1,0,'R','F');
                                                        $pdf->Cell(25,12,'',1,0,'R','F');
                                                      $pdf->Cell(26,12,$total_amt2,1,1,'R','F');
 ...
 $pdf->Ln(5);
                                              ...
                                              $pdf->SetFont('Arial','',10);
                                                     ...
                                                     $pdf->SetX(45); 
                                                     $pdf->Cell(90,8,ucwords(convert_number_to_words($total_bal))." Rupees Only",2,1,'L');
                                                     $pdf->SetX(2);        $pdf->Cell(45,8,'Amount Paid :',2,0,'L');

                                                     if($amt_paid != '0'){
                                                     $pdf->SetX(45);     $pdf->Cell(90,8,ucwords(convert_number_to_words($total_amt2))." Rupees Only",2,1,'L');
                                                       } else {
                                                       ...
                                                       $pdf->SetFont('Arial','B',10);
                                                     $pdf->SetX(45);     $pdf->Cell(90,8,ucwords(convert_number_to_words($balamt))." Rupees Only",2,1,'L');
