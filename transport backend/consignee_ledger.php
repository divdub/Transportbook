...
if($consignee_id !='' ) {
  $cond .= "and category_id='$consignee_id'"; 
    ...
    $cond1 .= "and catname='$consignee_id'"; 
}
	 ...
	 $currdate_str = strtotime($fromdate);
		...
		$currdate = date('Y-m-d', strtotime('-1 day', strtotime($fromdate)));
		      ...
		      $payment = $cmn->getvalfield($connection,"payment","sum(amt_paid_to)","consignorid=$consignorid && comp_id=$comp_id && category_id=2 && session_id=$session_id && voucher_date <= '$currdate' and catname='$consignee_id'"); 
		      $payment_receive = $cmn->getvalfield($connection,"payment_receive","sum(receive_amt)","consignorid=$consignorid && comp_id=$comp_id && category=2 && session_id=$session_id && receive_date <= '$currdate' and catname='$consignee_id'"); 
...
<head>
	...
	<title> CONSIGNEE LEDGER REPORT  :: CHAARUVI INFOTECH PVT. LTD.</title>
				...
				<div class="row">
					<div class="col-sm-12">
						<div class="box box-bordered box-color satblue">
							<div class="box-title">
								<h3>
									<i class="fa fa-list"></i>Consignee Ledger </h3>
							</div>
    ...
    <!--                          &nbsp;-->
			...
			<div class="box-content nopadding">
					...
					<tbody>
					 ...
					 <?php
									$sn=1;
// 			echo  "Select * from  payment  where consignorid=$consignorid && comp_id=$comp_id && category_id=2 $cond && session_id=$session_id GROUP BY voucher_id order by payment_id desc ";
				$sql = mysqli_query($connection,"Select * from  payment  where consignorid=$consignorid && comp_id=$comp_id && category_id=2 $cond && session_id=$session_id GROUP BY voucher_id order by payment_id desc ");
										  while($row= mysqli_fetch_array($sql)) {
...
$amt_paid_to=$cmn->getvalfield($connection,"payment","sum(amt_paid_to)","voucher_id='$row[voucher_id]' && consignorid=$consignorid");
				...
				$sql = mysqli_query($connection,"Select * from  payment_receive   where category=2 && consignorid=$consignorid && comp_id=$comp_id $cond1 && session_id=$session_id order by pay_receive_id desc");
										  while($row= mysqli_fetch_array($sql)) {
										      	$paid_to=$cmn->getvalfield($connection,"payment","payee_name","voucher_id='$row[voucher_no]' && consignorid='$consignorid' ");
