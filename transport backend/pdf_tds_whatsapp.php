...
class PDF_MC_Table extends FPDF
{
    ...
    $this->Cell(2,24,$mobileno1 .",". $mobileno2." ".$emailid, 0, 0, 'C');
        ...
        $this->SetY(28);
        $this->Cell(90);
        $this->Cell(10, 28, "TDS DETAILS", 0, 1, 'C');
        $this->Ln(5);
        ...
        $this->Ln(7);
              $this->Cell(12,8,"From : ". dateformatindia($fromdate)." To : ".dateformatindia($todate), 0, 1, 'L');
        // $this->SetX(8);
        $this->SetFont('Arial', 'B', 8);
        $this->SetFillColor(170, 170, 170); //gray
        $this->SetTextColor(0, 0, 0);  $this->SetX(5);
        $this->Cell(8, 7, 'Sno', '1', 0, 'L', 1);
        $this->Cell(15, 7, ' DI/LR No.', 1, 0, 'L', 1);
       
        // $this->Cell(30, 7, 'Truck No', 1, 0, 'L', 1);
        // $this->Cell(30, 7, 'Driver Name', 1, 0, 'L', 1);
        $this->Cell(17, 7, 'Bilty Date', 1, 0, 'L', 1);
         // $this->Cell(20, 7, 'Maintenance / Spare', 1, 0, 'L', 1);
 
        $this->Cell(38, 7, 'Paid To ', 1, 0, 'L', 1);
 $this->Cell(48, 7, 'Truck owner', 1, 0, 'C', 1);
    // $this->Cell(20, 7, 'Truck No', 1, 0, 'C', 1);
     $this->Cell(21, 7, 'Pan No', 1, 0, 'C', 1);
     $this->Cell(15, 7, 'Freight', 1, 0, 'C', 1);
      $this->Cell(20, 7, 'TDS Amt', 1, 0, 'C', 1);
      $this->Cell(18, 7, 'Amount ', 1, 1, 'C', 1);
     
        $this->SetWidths(array(8,15,17,38,48,21,15,20,18));
        
        $this->SetAligns(array("L", "L", "L","L","L","C","L","C","L","L","L","L"));
...
$tamt = 0;
									$sn=1;
				// 			echo "Select * from  payment  $crit  && tds_amt!=0  && consignorid=$consignorid order by payment_id desc";die;
				
				$sql = mysqli_query($connection,"Select * from  payment  $crit  && tds_amt!=0  && consignorid=$consignorid order by payment_id desc");
	...
	$di_no=$cmn->getvalfield($connection,"dispatch_entry","di_no","dispatch_id=$row[dispatch_id]");
		$bilty_date=$cmn->getvalfield($connection,"dispatch_entry","bilty_date","dispatch_id=$row[dispatch_id]");
		   ...
		   $place_name=$cmn->getvalfield($connection,"m_place","place_name","place_id=$destination_id");
			...
			$tfrt+=$amt;
		$ttds+=	$tds_amt;							   
    $tamt += $row['amt_paid_to'];
     $pdf->SetX(5);

    // $service_datenext=dateformatindia($row['service_datenext']);
    $pdf->Row(array($sn++,$di_no,dateformatindia($bilty_date),$row['payee_name'],$owner_name,$row['panno'],$amt,$tds_amt,number_format(round($row['amt_paid_to']))));
}
 $pdf->SetX(5);
         $pdf->Cell(147, 8, "Total Amount", 1, 0, 'C');
            $pdf->Cell(15, 8, $tfrt, 1, 0, 'C');
               $pdf->Cell(20, 8, $ttds, 1,0, 'C');
          $pdf->Cell(18, 8, $tamt, 1, 1, 'C');
