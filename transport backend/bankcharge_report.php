				...
				<div class="row">
					<div class="col-sm-12">
						<div class="box box-bordered box-color satblue">
							<div class="box-title">
								<h3>
									<i class="fa fa-list"></i>Bank Charge Report</h3>
							</div>
	...
	<!-- 									<div class="col-sm-3">
		...
		<!-- 		
			...
			<div class="box-content nopadding"  style="overflow:scroll;">
				...
				$sql = mysqli_query($connection,"Select * from  $tblname  $crit  && bank_charge!=0  && consignorid=$consignorid order by $tblpkey desc");
										  while($row= mysqli_fetch_array($sql)) {
		  ...
		  // $owner_name=$cmn->getvalfield($connection,"m_vehicle_owner","owner_name","owner_id=$owner_id");
		   ...
		   $place_name=$cmn->getvalfield($connection,"m_place","place_name","place_id=$destination_id");
			...
			$wt_mt=$cmn->getvalfield($connection,"dispatch_entry","wt_mt","dispatch_id=$row[dispatch_id]");
				...
				$amt=$comp_rate*$wt_mt;
			$totbank_charge +=$row['bank_charge'];
										   ?>
					<tr>
					<td><?php echo $sn++;?></td>
					...
					<td><?php echo $qty; ?></td>
						<td><?php echo number_format(round(($row['bank_charge']),2)); ?></td>
