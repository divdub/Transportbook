...
class PDF_MC_Table extends FPDF
{
    ...
    $this->Cell(2,24,$mobileno1 .",". $mobileno2." ".$emailid, 0, 0, 'C');
        ...
        $this->SetY(28);
        $this->Cell(90);
        $this->Cell(10, 28, "VEHICLE DETAILS", 0, 1, 'C');
        $this->Ln(5);
        ...
        $this->Cell(27, 7, 'OWNER NAME ', 1, 0, 'L', 1);
        $this->Cell(22, 7, 'VEHICLE NO', 1, 0, 'L', 1);
        $this->Cell(21, 7, 'VEHICLE TYPE', 1, 0, 'L', 1);
         $this->Cell(25, 7, 'AGENT NAME', 1, 0, 'L', 1);
        $this->Cell(20, 7, 'CHASSIS NO.', 1, 0, 'L', 1);
      $this->Cell(20, 7, 'ENGINE NO.', 1, 0, 'L', 1); 
       $this->Cell(22, 7, 'MTR READING', 1, 0, 'L', 1); 
   $this->Cell(30, 7, 'MTR READING DATE', 1, 1, 'L', 1); 
        $this->SetWidths(array(9, 27, 22,21,25,20,20,22,30));
...
$slno = 1;
// echo "select * from session_master where session_id='$session_id'";
...
$owner_name=$cmn->getvalfield($connection,"m_vehicle_owner","owner_name","owner_id=$row_get[owner_id]");
 $agent_name=$cmn->getvalfield($connection,"m_agent","agent_name","agent_id=$row_get[agent_id]");
$vehicle_type=$cmn->getvalfield($connection,"m_vehicle_type","vehicle_type","vehicle_type_id=$row_get[vehicle_type_id]");
$no_of_wheels=$cmn->getvalfield($connection,"m_vehicle_type","no_of_wheels","vehicle_type_id=$row_get[vehicle_type_id]");
    $meter_read_date=dateformatindia($row_get['meter_read_date']);
    $pdf->Row(array($slno++, $owner_name, $row_get['vehicle_no'],$vehicle_type."-".$no_of_wheels,$agent_name,$row_get['chassis_no'],$row_get['engine_no'],$row_get['meter_read'],$meter_read_date));
}
$pdf->Output();
?> 
