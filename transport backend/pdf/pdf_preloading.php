...
$pdf->SetFont('Arial','ub',16);
	...
	$pdf->SetTextColor(0,0,0);
	$pdf->Cell(280,4,$cname,'0',1,'C',0);
	$pdf->Ln(2);
		
		$pdf->SetFont('Arial','UB',10);
	$pdf->SetTextColor(0,0,0);
	$pdf->Cell(280,4,$caddress,'0',1,'C',0);
	$pdf->Ln(4);
	...
	$pdf->SetTextColor(0,0,0);
	$pdf->Cell(40,4,"PAN :-",'0',0,'R',0);
	$pdf->Cell(80,4,"$companypan",'0',0,'L',0);
	$pdf->Ln(2);
		$pdf->setX(200);
	$pdf->SetFont('Arial','',12);
	$pdf->SetTextColor(0,0,0);
	$pdf->Cell(40,4,"GSTN:-",'0',0,'R',0);
...
$pdf->Image('../upload/logo/'.$c_logo,6,6,18,11);
	...
	$pdf->SetFont('Arial','',11);
	$pdf->SetTextColor(0,0,0);
	$pdf->Cell(40,4,"TRANSPORT CODE:",'0',0,'R',0);
...
$pdf->Ln(2);
	...
	$pdf->Cell(18,6,'INV. NO','1',0,'L',1);  
	$pdf->Cell(17,6,' DATE',1,0,'L',1);
		$pdf->Cell(18,6,'DI NO ',1,0,'L',1);
			$pdf->Cell(20,6,'TRUCK NO ',1,0,'L',1);	
    $pdf->Cell(21,6,'GR NO',1,0,'L',1);

	$pdf->Cell(30,6,'CONSIGNEE NAME',1,0,'L',1);
	$pdf->Cell(20,6,'STATION',1,0,'L',1);
	$pdf->Cell(12,6,'GRADE',1,0,'L',1);
	$pdf->Cell(16,6,'DES QTY',1,0,'L',1);
	$pdf->Cell(16,6,'FRT RATE.',1,0,'L',1);
	$pdf->Cell(15,6,'FRT AMT',1,0,'L',1);
	

		$pdf->Cell(15,6,'IGST '.$gst.'%',1,0,'L',1);
	$pdf->Cell(15,6,'CGST'.$gst/2 .'%',1,0,'L',1);
	$pdf->Cell(15,6,'SGST'.$gst/2 .'%',1,0,'L',1);
	$pdf->Cell(16,6,'UTGST 6%',1,0,'L',1);
		$pdf->Cell(15,6,'GST TTL',1,1,'C',1);
...
// 	$pdf->Cell(10,6,'MT',1,1,'L',1);
	...
	$pdf->SetWidths(array(8,18,17,18,20,21,30,20,12,16,16,15,15,15,15,16,15));
...
while($row2=mysqli_fetch_assoc($sql2)) {
	...
	$pdf->SetFont('Arial','',8);
	$pdf->SetTextColor(0,0,0);
	$pdf->Row(array($sn++,$invoiceno,$cmn->dateformatindia($gr_date),$di_no,$truckno,$gr_no,$consignee_name,$deliverat,'',$totalweight,$comp_rate,number_format($total_amt,2),number_format($igst,2),number_format($gst1,2),number_format($gst1,2),'',number_format($gstamt,2)));
	...
	$pdf->SetTextColor(0,0,0);
	$pdf->Cell(164,4,"",'1',0,'R',0);
	$pdf->Cell(16,4,number_format($tot_wt,2),'1',0,'L',0);
	$pdf->Cell(16,4,'','1',0,'L',0);
	$pdf->Cell(15,4,number_format($nettotal_amt,2),'1',0,'L',0);
	
// 		$pdf->Cell(20,4,'','1',0,'L',0);
	
	$pdf->Cell(15,4,number_format($totgst,2),'1',0,'L',0);	
	$pdf->Cell(15,4,number_format($tot_gst1 ,2),'1',0,'L',0);
	$pdf->Cell(15,4,number_format($tot_gst1,2),'1',0,'L',0);
		$pdf->Cell(16,4,number_format(0,2),'1',0,'L',0);
	$pdf->Cell(15,4,number_format($tot_gst,2),'1',1,'L',0);
...
$pdf->Cell(210);
	...
	$pdf->SetFont('Arial','',8);
	$pdf->SetTextColor(0,0,0);
	$pdf->MultiCell(157,3,"1 .I/we have taken registration under the CGST Act, 2017 and have exercised the option to pay tax for :	on services of GTA in relation to transport of goods supplied by us during the Financial Year 2026-2027 under forward charge	only",'0',1,'L',0); 
