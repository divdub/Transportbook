...
class PDF_MC_Table extends FPDF
{
    ...
    $this->Cell(2,24,$mobileno1 .",". $mobileno2." ".$emailid, 0, 0, 'C');
        ...
        $this->SetY(28);
        $this->Cell(90);
        $this->Cell(10, 28, "RETURN TRIP DETAILS", 0, 1, 'C');
        $this->Ln(5);
        ...
        $this->Cell(15, 7, 'TRIP No.', 1, 0, 'L', 1);
        // $this->Cell(10, 7, 'BILTY NO. ', 1, 0, 'L', 1);
        $this->Cell(18, 7, 'LOAD DATE', 1, 0, 'L', 1);
         $this->Cell(30, 7, 'CONSIGNOR', 1, 0, 'L', 1);
        $this->Cell(30, 7, 'CONSIGNEE', 1, 0, 'L', 1);
        $this->Cell(22, 7, 'TRUCK NO.', 1, 0, 'L', 1);
        $this->Cell(20, 7, 'DESTINATION', 1, 0, 'R', 1);
        $this->Cell(11, 7, 'WT/MT', 1, 0, 'L', 1);
        $this->Cell(10, 7, 'RATE', 1,0 , 'L', 1);
        $this->Cell(16, 7, 'CASH ADV', 1, 0, 'L', 1);
        $this->Cell(18, 7, 'DIESEL ADV', 1, 1, 'L', 1);

        $this->SetWidths(array(8, 15,18,30,30,22,20,11,10,16,18));
...
$slno = 1;
$sql = mysqli_query($connection, "select * from trip_entry $crit && sessionconsignor_id=$consignorid");
while ($row_get = mysqli_fetch_array($sql)) {
    
    $pdf->SetX(6);
    $consignor_name=$cmn->getvalfield($connection,"m_party","party_name","party_id=$row_get[consignor_id]");
	$consignee_name=$cmn->getvalfield($connection,"m_party","party_name","party_id=$row_get[consignee_id]");
	$vehicle_no=$cmn->getvalfield($connection,"m_vehicle","vehicle_no","vehicle_id=$row_get[vehicle_id]");
$destination=$cmn->getvalfield($connection,"m_place","place_name","place_id=$row_get[toplaceid]");	        
    $loding_date=dateformatindia($row_get['loding_date']);
    $pdf->Row(array($slno++, $row_get['trip_no'],$loding_date,$consignor_name,$consignee_name,$vehicle_no,$destination,$row_get['qty_mt_day_trip'],$row_get['rate'],$row_get['cash_advance'],$row_get['diesel_advance']));
