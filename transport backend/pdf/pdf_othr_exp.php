...
class PDF_MC_Table extends FPDF
{
    ...
    $this->Cell(2,24,$mobileno1 .",". $mobileno2." ".$emailid, 0, 0, 'C');
        ...
        $this->SetY(28);
        $this->Cell(90);
        $this->Cell(10, 28, "OTHER EXPENSE DETAILS", 0, 1, 'C');
        $this->Ln(5);
        ...
        $this->Cell(40, 7, 'Other Expense', 1, 0, 'L', 1);
 ...
 $this->Cell(50, 7, 'Narration', 1, 0, 'C', 1);
    $this->Cell(40, 7, 'Amount', 1, 1, 'C', 1);
        $this->SetWidths(array(8, 20,40,30,50,40));
        $this->SetAligns(array("L", "L", "L","L","L","C","L","C"));
    }
    // Page footer
    function Footer()
    {
...
$sql = mysqli_query($connection, "select * from other_expense_entry $crit");
while ($row = mysqli_fetch_array($sql)) {
    ...
    $exp_date=dateformatindia($row['exp_date']);
    $tamt += $row['amount'];
    // $service_datenext=dateformatindia($row['service_datenext']);
    $pdf->Row(array($slno++,$exp_date,$head_name,$row['bill_type'],$row['narration'],$row['amount']));
}
