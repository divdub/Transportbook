...
class PDF_MC_Table extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Image('../upload/logo/'.$clogo,15,6,12,12);
    ...
    $this->Cell(2,24,$mobileno1 .",". $mobileno2." ".$emailid, 0, 0, 'C');
        ...
        $this->SetY(28);
        $this->Cell(90);
        $this->Cell(10, 28, "TPA DETAILS", 0, 1, 'C');
        $this->Ln(5);
        ...
        $this->Cell(15, 7, 'DI No', 1, 0, 'L', 1);
           $this->Cell(20, 7, 'BILTY DATE', 1, 0, 'L', 1);
        $this->Cell(20, 7, 'Truck No.', 1, 0, 'L', 1);
         $this->Cell(25, 7, 'Destination', 1, 0, 'L', 1);
         $this->Cell(20, 7, 'CATEGORY', 1, 0, 'C', 1);
        $this->Cell(50, 7, 'CATEGORY NAME', 1, 0, 'C', 1);
          $this->Cell(10, 7, 'WT/MT', 1, 0, 'C', 1);
        $this->Cell(10, 7, 'RATE', 1, 0, 'C', 1);
        $this->Cell(12, 7, 'AMOUNT', 1, 1, 'C', 1);
        // $this->Cell(18, 7, 'OTHR ADV', 1, 0, 'L', 1);
        // $this->Cell(26, 7, 'CONSIGNOR ADV', 1,0 , 'L', 1);
        // $this->Cell(26, 7, 'CONSIGNEE ADV', 1, 1, 'L', 1);
        // $this->Cell(18, 7, 'COMP RATE', 1, 1, 'L', 1);

        $this->SetWidths(array(10, 15,20,20,25,20,50,10,10,12,15,15));
        $this->SetAligns(array("L", "L", "C","C","C","C","C","C","L","L"));
    }
    // Page footer
    function Footer()
    {
...
$slno = 1;
$sql = mysqli_query($connection, "select * from tpa_entry $crit && is_create=0 && consignorid=$consignorid");
while ($row_get = mysqli_fetch_array($sql)) {
   $pdf->SetX(10);
    $category=$row_get['tpcat_id'];
    $wt_mt=$cmn->getvalfield($connection,"dispatch_entry","wt_mt","dispatch_id=$row_get[dispatch_id]");
     $vehicle_id=$cmn->getvalfield($connection,"dispatch_entry","vehicle_id","dispatch_id=$row_get[dispatch_id]");
     $destination_id=$cmn->getvalfield($connection,"dispatch_entry","destination_id","dispatch_id=$row_get[dispatch_id]");
...
if($category==1){
    $c_name="Agent";
    $catname = $cmn->getvalfield($connection,"m_agent","agent_name","agent_id = '$row_get[category_id]'");

} 
if($category==2){
    $c_name="Consignee";
    $catname = $cmn->getvalfield($connection,"m_consignee","consignee_name","consignee_id = '$row_get[category_id]'");

} 
if($category==4) {
    $c_name="Truck Owner";
        $catname = $cmn->getvalfield($connection,"m_vehicle_owner","owner_name","owner_id = '$row_get[category_id]'");
}
         
    $bilty_date=dateformatindia($row_get['bilty_date']);
    $tamt +=$row_get['amt'];
    
    $pdf->Row(array($slno++, $row_get['di_no'],$bilty_date,$vehicle_no,$destination,$c_name,$catname,$wt_mt,$row_get['rate'],$row_get['amt']));
}
   $pdf->Cell(160, 7, 'Total Amount : ', '1', 0, 'L');
 $pdf->Cell(32, 7, $tamt, '1', 1, 'R');
  $pdf->Ln(7);
        $pdf->SetX(10);
        $pdf->SetFont('Arial', 'B', 16);
        // $pdf->SetFillColor(170, 170, 170); //gray
        // $pdf->SetTextColor(0, 0, 0);
        // $pdf->Cell(12, 7, 'Total Amount : ', '2', 0, 'L');
$pdf->Output();
?> 
