...
class PDF_MC_Table extends FPDF
{
    ...
    $this->Cell(2,24,$mobileno1 .",". $mobileno2." ".$emailid, 0, 0, 'C');
        ...
        $this->SetY(28);
        $this->Cell(90);
        $this->Cell(10, 28, "OTHER INCOME DETAILS", 0, 1, 'C');
        $this->Ln(5);
        ...
        $this->Cell(40, 7, 'Other Income', 1, 0, 'L', 1);
         // $this->Cell(20, 7, 'Maintenance / Spare', 1, 0, 'L', 1);
        //  $this->Cell(20, 7, 'Amount', 1, 0, 'L', 1);
        
        $this->Cell(30, 7, 'Payment Mode', 1, 0, 'L', 1);
 $this->Cell(50, 7, 'Narration', 1, 0, 'C', 1);
    $this->Cell(40, 7, 'Amount', 1, 1, 'C', 1);
        $this->SetWidths(array(8, 20, 40,30,50,40,40));
        $this->SetAligns(array("L", "L", "L","L","L","C","L"));
    }
    // Page footer
    function Footer()
    {
...
$sql = mysqli_query($connection, "select * from othr_inc_entry $crit");
while ($row = mysqli_fetch_array($sql)) {
    ...
    $inc_date=dateformatindia($row['inc_date']);
    // $service_datenext=dateformatindia($row['service_datenext']);
     $tamt += $row['amount'];
    $pdf->Row(array($slno++,$inc_date,$head_name,$row['bill_type'],$row['narration'],$row['amount']));
}
 $pdf->Cell(148, 8, "Total Amount", 1, 0, 'C');
          $pdf->Cell(40, 8, $tamt, 1, 1, 'C');
$pdf->Output();
?> 
