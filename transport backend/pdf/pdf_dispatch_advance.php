...
class PDF_MC_Table extends FPDF
{
    ...
    $this->Cell(2,24,$mobileno1 .",". $mobileno2." ".$emailid, 0, 0, 'C');
        ...
        $this->SetY(28);
        $this->Cell(90);
        $this->Cell(10, 28, "DISPATCH ADVANCE DETAILS", 0, 1, 'C');
        $this->Ln(5);
        ...
        $this->Cell(18, 7, 'BILTY DATE', 1, 0, 'L', 1);
         $this->Cell(20, 7, 'TRUCK NO', 1, 0, 'L', 1);
        $this->Cell(21, 7, 'FREIGHT AMT.', 1, 0, 'R', 1);
        $this->Cell(20, 7, 'DIESEL AMT', 1, 0, 'L', 1);
        $this->Cell(18, 7, 'CASH ADV', 1, 0, 'R', 1);
        ...
        $this->SetWidths(array(8, 18,18,20,21,20,18,18,26,26));
...
$slno = 1;
$sql = mysqli_query($connection, "select * from dispatch_entry where is_advance=1 && consignor_id=$consignorid");
while ($row_get = mysqli_fetch_array($sql)) {
    $pdf->SetX(6);
    $wt_mt =$row_get['wt_mt'];
     $own_rate=$row_get['own_rate'];
    ...
    $pdf->Row(array($slno++, $row_get['di_no'],$bilty_date,$vehicle_no,$freight_amt,$row_get['diesel_adv_amt'],$row_get['cash_adv'],$row_get['other_cash_adv'],$row_get['consignor_cash_adv'],$row_get['consignee_cash_adv']));
}
$pdf->Output();
?> 
