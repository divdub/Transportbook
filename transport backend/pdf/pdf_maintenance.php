...
class PDF_MC_Table extends FPDF
{
    ...
    $this->Cell(2,24,$mobileno1 .",". $mobileno2." ".$emailid, 0, 0, 'C');
        ...
        $this->SetY(28);
        $this->Cell(90);
        $this->Cell(10, 28, "MAINTENANCE DETAILS", 0, 1, 'C');
        $this->Ln(5);
        ...
        $this->Cell(40, 7, 'Mechanic Name', 1, 0, 'L', 1);
         $this->Cell(20, 7, 'Maintenance / Spare', 1, 0, 'L', 1);
         $this->Cell(20, 7, 'Amount', 1, 0, 'L', 1);
        
        $this->Cell(30, 7, 'Payment Mode', 1, 1, 'L', 1);

        $this->SetWidths(array(8, 20, 30,30,40,20,20,30));
...
$sql = mysqli_query($connection, "select * from maintenance_entry");
while ($row = mysqli_fetch_array($sql)) {
    ...
    $mdate=dateformatindia($row['mdate']);
    // $service_datenext=dateformatindia($row['service_datenext']);
    $pdf->Row(array($slno++,$mdate,  $vehicle_no,$driver_name,$mechanic_name,$head_name,$row['amount'],$row['payment_mode']));
}
$pdf->Output();
?> 
