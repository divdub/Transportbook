...
class PDF_MC_Table extends FPDF
{
    ...
    $this->Cell(2,24,$mobileno1 .",". $mobileno2." ".$emailid, 0, 0, 'C');
        ...
        $this->SetY(28);
        $this->Cell(90);
        $this->Cell(10, 28, "ITEM DETAILS", 0, 1, 'C');
        $this->Ln(5);
        ...
        $this->Cell(47, 7, 'ITEM NAME ', 1, 0, 'L', 1);
         $this->Cell(45, 7, 'CATEGORY NAME', 1, 0, 'L', 1);
        $this->Cell(20, 7, 'UNIT NAME', 1, 1, 'L', 1);
        $this->SetX(35);
        $this->SetWidths(array(9, 47,45,20));
...
$sql = mysqli_query($connection, "select * from  m_item");
while ($row_get = mysqli_fetch_array($sql)) {
    // $pdf->SetX(5);
     $pdf->SetX(35);
$unit_name=$cmn->getvalfield($connection,"m_unit","unit_name","unit_id=$row_get[unit_id]");
                    $category_name=$cmn->getvalfield($connection,"m_item_category","category_name","item_category_id=$row_get[item_category_id]");
    $pdf->Row(array($slno++, $row_get['item_name'],$category_name,$unit_name));
}
$pdf->Output();
?> 
