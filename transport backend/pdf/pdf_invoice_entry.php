...
class PDF_MC_Table extends FPDF
{
    ...
    $this->Cell(2,24,$mobileno1 .",". $mobileno2." ".$emailid, 0, 0, 'C');
        ...
        $this->SetY(28);
        $this->Cell(90);
        $this->Cell(10, 28, "INVOICE ENTRY DETAILS", 0, 1, 'C');
        $this->Ln(5);
        ...
        $this->Cell(17, 7, 'Bill Type', 1, 0, 'L', 1);
        $this->Cell(30, 7, 'INVOICE NO', 1, 0, 'L', 1);
        
        $this->Cell(24, 7, 'INVOICE DATE', 1, 0, 'L', 1);
        $this->Cell(14, 7, 'QTY', 1, 0, 'L', 1);
        $this->Cell(20, 7, 'AMOUNT', 1, 0, 'L', 1);
        $this->Cell(30, 7, 'Invoice Amount', 1, 0, 'L', 1);
        $this->Cell(20, 7, 'Status', 1, 0, 'L', 1);
        $this->Cell(30, 7, 'Received Amount', 1, 1, 'L', 1);
      

        $this->SetWidths(array(8, 17,30,24,14,20,30,20,30));
        $this->SetAligns(array("L", "L", "L","L","L","L","L","L","L"));
        // $this->Cell(93, 7, 'Total', 1, 0, 'C', 0);
        // $this->Cell(20, 7, $totalamount, 1, 0, 'C', 0);
        // $this->Cell(30, 7, $totalamount1, 1, 0, 'C', 0);
        // $this->Cell(20, 7, 'total=', 1, 0, 'C', 0);
        // $this->Cell(30, 7, 'total', 1, 1, 'C', 0);
    }
    // Page footer
    function Footer()
    {
...
$totalamount1 = 0;
$totalinvamt = 0;
// echo "select * from invoicebilty $crit and consignorid=$consignorid and sessionid=$session_id";die;
$sql = mysqli_query($connection, "select * from invoicebilty $crit and consignorid=$consignorid and sessionid=$session_id");
while ($row_get = mysqli_fetch_array($sql)) {
    $pdf->SetX(10);
     $amount = $cmn->getinvoiceamount($connection,$row_get['invoiceid']);
     $wt_mt = $cmn->getvalfield($connection,"dispatch_entry","sum(wt_mt)","invoiceid='$row_get[invoiceid]'");  
     $invoiceAmt  = $cmn->getvalfield($connection,"manualinv","sum(received_amt)","invoiceid='$row_get[invoiceid]'"); 
    $invdate=dateformatindia($row_get['invdate']);
    $gstv = $row['gst'];
    $gst =  ($amount * $gstv)/100;
  	$amount1=$amount+$gst;
  	$totalamount +=$amount;
    ...
    $pdf->Row(array($slno++, $row_get['billtype'],$row_get['invno'],$invdate,$wt_mt,$amount,$amount1,$status,$invoiceAmt));
}
$pdf->SetFont('Arial','B',9);
$pdf->SetX(10);

$pdf->Cell(93,8,'Total',1,0,'C');
$pdf->Cell(20,8,$totalamount,1,0,'C');
$pdf->Cell(30,8,$totalamount1,1,0,'C');
$pdf->Cell(20,8,'',1,0,'C');
$pdf->Cell(30,8,$totalinvamt,1,1,'C');
$pdf->SetFont('Arial','B',9);
$pdf->Output();
?> 
