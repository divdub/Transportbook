...
class PDF_MC_Table extends FPDF
{
    ...
    $this->Cell(2,24,$mobileno1 .",". $mobileno2." ".$emailid, 0, 0, 'C');
        ...
        $this->SetY(28);
        $this->Cell(90);
        $this->Cell(10, 28, "EMPLOYEE PAYMENT DETAILS", 0, 1, 'C');
        $this->Ln(5);
        ...
        $this->Cell(40, 7, 'PAYMENT DATE', 1, 0, 'L', 1);
        $this->Cell(40, 7, 'EMPLOYEE NAME ', 1, 0, 'L', 1);
        $this->Cell(30, 7, 'EXPENSE NAME', 1, 0, 'L', 1);
        $this->Cell(30, 7, 'AMOUNT', 1, 0, 'L', 1);
         $this->Cell(30, 7, 'PAY MODE', 1, 1, 'L', 1);
        


        $this->SetWidths(array(10,40,40, 30,30,30));
...
while ($row_get = mysqli_fetch_array($sql)) {
    // $pdf->SetX(5);
     ...
     $employee_name=$cmn->getvalfield($connection,"m_employee","employee_name","employee_id=$row_get[employee_id]");

   $head_name=$cmn->getvalfield($connection,"inc_exp_head","head_name","inc_exp_id=$row_get[inc_exp_id]");
    $payment_date=dateformatindia($row_get['payment_date']);
    $pdf->Row(array($slno++,$payment_date,$employee_name , $head_name ,$row_get['amount'],$row_get['pay_mode']));
}
$pdf->Output();
?> 
