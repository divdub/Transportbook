...
class PDF_MC_Table extends FPDF
{
    ...
    function Header()
    {
       ...
       $this->SetFont('courier', 'b', 12);
        ...
        $this->SetY(28);
        $this->Cell(90);
        $this->Cell(10, 28, "BILL PAYMENT DETAILS", 0, 1, 'C');
        $this->Ln(5);
        ...
        $this->Cell(25, 7, 'INVOICE NO', 1, 0, 'L', 1);
        $this->Cell(25, 7, 'RECEIVE DATE', 1, 0, 'L', 1);
         $this->Cell(25, 7, 'TDS AMT', 1, 0, 'L', 1);
           $this->Cell(26, 7, 'GST AMT ', 1, 0, 'L', 1);
             $this->Cell(25, 7, 'DEDUCT AMT ', 1, 0, 'L', 1);
        $this->Cell(26, 7, 'RECEIVED AMT', 1, 0, 'L', 1);
        $this->Cell(30, 7, 'REMARK', 1, 1, 'L', 1);

        $this->SetWidths(array(8, 25,25,25,26,25,26,30));
...
$total_deduct= 0; 
$total_received= 0; 
$sql = mysqli_query($connection, "select * from manualinv");
while ($row_get = mysqli_fetch_array($sql)) {
    // $pdf->SetX(35);
      if ($row['type'] == 'incentive') {
        
          $invno = $row_get['ref_no'];
       }else{
          $invno = $cmn->getvalfield($connection, "invoicebilty", "invno", "invoiceid='" . $row_get['invoiceid'] . "'");
       }	
    
    $total_tdsamt += $row_get['tds_amt'];
	$total_gstamt += $row_get['gst_amt'];
	$total_deduct += $row_get['deduct'];
	$total_received += $row_get['received_amt'];
           
    $receive_date=dateformatindia($row_get['receive_date']);
    $pdf->Row(array($slno++, $invno,$receive_date,$row_get['tds_amt'],$row_get['gst_amt'],$row_get['deduct'],$row_get['received_amt'],$row_get['remark']));
}

$pdf->SetFont('Arial', 'B', 9);
$pdf->Row(array(
    '', '', 'TOTAL',
    number_format($total_tdsamt, 2),
    number_format($total_gstamt, 2),
    number_format($total_deduct, 2),
