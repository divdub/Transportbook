...
class PDF_MC_Table extends FPDF
{
    ...
    $this->Cell(2,24,$mobileno1 .",". $mobileno2." ".$emailid, 0, 0, 'C');
        ...
        $this->SetY(28);
        $this->Cell(90);
        $this->Cell(10, 28, "PAYMENT DETAILS", 0, 1, 'C');
        $this->Ln(5);
        ...
        $this->Cell(25, 7, 'RECEIVE No', 1, 0, 'L', 1);
        $this->Cell(25, 7, 'PAYMENT DATE', 1, 0, 'L', 1);
         $this->Cell(20, 7, 'CATEGORY', 1, 0, 'C', 1);
        $this->Cell(30, 7, 'VOUCHER NAME', 1, 0, 'C', 1);
        ...
        $this->SetWidths(array(12, 25,25,25,20,30,25,18,26,26));
...
$slno = 1;
// echo "select * from payment_receive where consignorid=$consignorid order by pay_receive_id desc";
$sql = mysqli_query($connection, "select * from payment_receive where consignorid=$consignorid order by pay_receive_id desc");
while ($row_get = mysqli_fetch_array($sql)) {
   $pdf->SetX(25);
  
if($row_get['category']==1){
    $coname="Agent";
  
} 
if($row_get['category']==2){
    $coname="Consignee";

} 
if($row_get['category']==4) {
    $coname="Truck Owner";
   
}

         
    $receive_date=dateformatindia($row_get['receive_date']);
    $pdf->Row(array($slno++, $row_get['voucher_no'],$row_get['rec_no'],$receive_date,$coname,$row_get['voucher_name'], $row_get['receive_amt']));
}
$pdf->Output();
?> 
