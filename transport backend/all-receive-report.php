...
$tblpkey = "dispatch_id";
$pagename = "all-receive-report.php";
				...
				<div class="row">
					<div class="col-sm-12">
						<div class="box box-bordered box-color satblue">
							<div class="box-title">
								<h3>
									<i class="fa fa-list"></i>Dispatch Receive</h3>
							</div>
...
<form action="#" method="GET" class='form-horizontal form-column form-bordered'>
			...
			<div class="box-content nopadding"  style="overflow:scroll;">
				<table class="table table-hover table-nomargin table-striped table-bordered dataTable dataTable-colvis">
					...
					<tbody>
					 ...
					 <?php
									$sn=1;
				// 			echo	"Select * from  $tblname  $crit && is_receive=1 && consignor_id=$consignorid && comp_id=$comp_id && session_id=$session_id order by $tblpkey desc";
				$sql = mysqli_query($connection,"Select * from  $tblname  $crit && is_receive=1 && consignor_id=$consignorid && comp_id=$comp_id && session_id=$session_id order by $tblpkey desc");
										  while($row= mysqli_fetch_array($sql)) {
	...
	$user_name=$cmn->getvalfield($connection,"m_userlogin","user_name","user_id=$row[recuser_id]");
					...
					<td><?php echo $sn++;?></td>
						...
						<td>
							<?php if($row['rec_img']!=''){ ?>
							<a href="ajax/image/<?php echo $row['rec_img']; ?>" target="_blank" download><img src="img/download.png" style="width: 30%;"/></a>
							<?php }else{ ?>
                             <?php echo "No Image" ?>
							<?php } ?>
						</td>
