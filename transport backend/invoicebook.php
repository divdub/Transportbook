...
//   $cond = "and inc_date between '$fromdate' and '$todate' "; 
    $cond1 = "WHERE receive_gstdate between '$fromdate' and '$todate' "; 
 $cond= "WHERE receive_date BETWEEN '$fromdate' AND '$todate'";
//   $cond1 =" WHERE other_expense_entry.exp_date BETWEEN '$fromdate' AND '$todate'";
  $cond2="WHERE invdate BETWEEN '$fromdate' AND '$todate'";
}
...
// 	$pdf->Cell(24,6,'GR/TR NO',1,0,'L',1);
	...
	$pdf->Cell(50,6,'Bill No.',1,0,'C',1);	
	$pdf->Cell(60,6,'Bill Amount ',1,0,'C',1);	
...
// 		$pdf->Cell(25,6,'INCOME',1,0,'C',1);
		$pdf->Cell(40,6,'Receive Amount',1,0,'C',1);
...
//  ORDER BY invdate;";
                    $sql = mysqli_query($connection, "SELECT invno,invdate,invoiceid,for_ledger,itemtype,NULL AS type,
             NULL AS ref_no,NULL AS deduct
FROM invoicebilty $cond2 and sessionid='$session_id'  && consignorid=$consignorid 
UNION 
SELECT invoiceid,receive_date, minvid ,for_ledger,received_amt,type,ref_no,deduct
FROM manualinv $cond and session_id='$session_id'  && consignorid=$consignorid
UNION 
SELECT invoiceid,receive_gstdate, minvid ,forledger,received_gstamt,type,ref_no,NULL AS deduct
FROM manualinv $cond1 and session_id='$session_id'  && consignorid=$consignorid
 ORDER BY invdate");
...
while ($row = mysqli_fetch_array($sql)) {
    ...
    if ($type == '') {
        $particular = "Receive Payment";
        $invno = $cmn->getvalfield($connection, "invoicebilty", "invno", "invoiceid='$row[invno]'");
        $received_amt = $row['itemtype'];
        $balamt = $totbalamt - $received_amt;
        $amount1 = '0';
    }else if($type == 'incentive'){
        $particular = "Incentive Amount";
        $invno = $row['ref_no'];
        $received_amt = $row['itemtype'];
        $balamt = $totbalamt + $received_amt;
        $amount1 = '0';
    }else if ($type == 'Deduct') {
        $particular = "Deduct Amount";
        $invno = $row['ref_no'];
        $received_amt = $row['deduct'];
        $balamt = $totbalamt - $received_amt;
        $amount1 = '0';
    }
     else if ($type == '0') {
        $particular = 'Freight Bill';
        $invno = $row['invno'];
        $amount = $cmn->getinvoiceamount1($connection, $row['invoiceid']);
        $gstval = $cmn->getvalfield($connection, "invoicebilty", "gst", "invoiceid='$row[invoiceid]'");
        $gst =  ($amount * $gstval) / 100;
        $amount1 = $amount + $gst;
        $received_amt = '0';
        ...
        if($inc == 'incentive'){
           $particular = 'Incentive GST Payment';
        $invno = $row['ref_no'];
        $received_amt = $row['itemtype'];
        $balamt = $totbalamt + $received_amt;
        $amount1 = '0';
        }else{
        $particular = 'GST Payment';
        $invno = $cmn->getvalfield($connection, "invoicebilty", "invno", "invoiceid='$row[invno]'");
        $received_amt = $row['itemtype'];
        $balamt = $totbalamt - $received_amt;
        $amount1 = '0';
...
//                 
      ...
      $pdf->SetX(15);
                                                ...
                                                $pdf->Cell(19,8,date('d-m-Y',strtotime($row['invdate'])),1,0,'R');
                                                                                        // $pdf->Cell(24,8, $gr_no, 1,0, 'R');
                                                                                        
                                                    $pdf->Cell(30,8, $particular, 1,0, 'R');
                                                       $pdf->Cell(50,8, $invno, 1,0, 'C');
                                                    $pdf->Cell(60,8, $amount1, 1,0, 'R');
                                                    // $pdf->Cell(28,8, $deliverat, 1,0, 'R');
                                                    //      $pdf->Cell(13,8, $slip_no,1,0,'L');
                                                    //  $pdf->Cell(15,8, $dbillno, 1,0, 'R');
                                             
                                              $pdf->Cell(40,8,$received_amt,1,0,'C');

                                               $pdf->Cell(60,8,$balamt,1,0,'C');
                      ...
                      $pdf->Ln();
                                                                ...
                                                                $total_tds+=$tds_amt;
                                                                    ...
                                                                    // $totbalamt =  $total_incamount  -$total_expense_amount ;
                                                                    $totbalamt  = $balamt;
                                            }    
       ...
       $pdf->SetX(15);
                                        ...
                                        // $pdf->Line($currentX , $currentY + 16, $currentX + 293, $currentY + 16); 
                                           $pdf->SetFillColor(175, 213, 240);
                                                     ...
                                                     $pdf->SetFont('Arial','B',9);
                                                     $pdf->Cell(107,12,'Total',1,0,'C','F');

                                                     
                                                  
                                                      $pdf->SetFont('Arial','B',8);
                                                    //   $pdf->Cell(20,12,$total_final,1,0,'R','F');
                                                        $pdf->Cell(60,12,$total_incamount,1,0,'R','F');
                                                      $pdf->Cell(40,12,$total_expense_amount,1,0,'R','F');
   $pdf->Cell(60,12,$totbalamt,1,1,'R','F');
                                                    ...
                                                    //  $pdf->Cell(90,8,ucwords(convert_number_to_words($total_final))." Rupees Only",2,1,'L');
                                                     ...
                                                     $pdf->SetX(55);     $pdf->Cell(90,8,ucwords(getinwordsbyindia($total_incamount))." Rupees Only",2,1,'L');
                                                       $pdf->SetX(10);
                                                       
                                                         $pdf->Cell(45,8,'Total Receive Amount : ',2,0,'L');

                                                     $pdf->SetX(55); 
                                                     $pdf->Cell(90,8,ucwords(getinwordsbyindia($total_expense_amount))." Rupees Only",2,1,'L');
                                                      $pdf->SetX(10);      $pdf->Cell(45,8,'Balance   :',2,0,'L');

                                                      $pdf->SetFont('Arial','B',10);
                                                     $pdf->SetX(55);     $pdf->Cell(90,8,ucwords(getinwordsbyindia($totbalamt))." Rupees Only",2,1,'L');
