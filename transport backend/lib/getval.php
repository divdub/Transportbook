...
function getinvoiceamount1($connection,$invoiceid)
{
$nettotal=0;

//echo "select own_rate,totalweight,mul(own_rate * totalweight) as total from bidding_entry where invoiceid='$invoiceid'"; die;
	$sql = mysqli_query($connection,"select comp_rate,wt_mt,sum(comp_rate * wt_mt) as total from dispatch_entry where invoiceid='$invoiceid'");
...
//echo "select own_rate,totalweight,mul(own_rate * totalweight) as total from bidding_entry where invoiceid='$invoiceid'"; die;
	$sql = mysqli_query($connection,"select own_rate,wt_mt,sum(own_rate * wt_mt) as total from dispatch_entry where invoiceid='$invoiceid'");
	while($row=mysqli_fetch_assoc($sql))
	{
	$total=0;
	//print_r($row);
...
//echo $currdate; die;
	...
	//echo $currdate; die; 
		 ...
		 $cementadv = $this->getvalfield($connection,"bidding_entry","sum(adv_cash)","itemid=2 && adv_date between '$open_bal_date' and '$currdate' and compid=$compid ");
					...
					$otherincome = $this->getvalfield($connection,"othr_inc_entry","sum(amount)","bill_type='Cash' && inc_date between '$open_bal_date' and '$currdate'  && consignorid='$consignorid'");  
						
						$otherexpense = $this->getvalfield($connection,"other_expense_entry","sum(amount)","bill_type='Cash' && exp_date between '$open_bal_date' and '$currdate'  && consignorid='$consignorid'");  
...
function getcashopeningplant($connection,$currdate,$comp_id,$consignorid,$session_id) {
//echo $currdate; die;
  $open_bal_date = $this->getvalfield($connection,"account_setting","open_bal_date","consignorid='$consignorid'");  
	...
	{
		// echo "hioooo"; 
		 $openingbal = $this->getvalfield($connection,"account_setting","cashopeningbal","consignorid='$consignorid' ");
		 
	}
	else
	{
	// echo "hihghh"; 
	$currdate1 = date('Y-m-d', strtotime('-1 day', strtotime($currdate)));
	
	//echo $currdate; die; 
		
		 $cashopeningbal = $this->getvalfield($connection,"account_setting","cashopeningbal","consignorid='$consignorid'");
		 $cementadv = $this->getvalfield($connection,"dispatch_entry","sum(cash_adv)","cash_adv_date between '$open_bal_date' and '$currdate1' and comp_id='$comp_id'  && consignor_id='$consignorid'");
		...
		// echo "othr_inc_entry as A left join m_userlogin as B on A.user_id = B.user_id","sum(amount)","bill_type='Cash' && inc_date between '$open_bal_date' and '$currdate'  && A.user_id in (select user_id from m_userlogin where comp_id=$comp_id)";
		$otherincome = $this->getvalfield($connection,"othr_inc_entry","sum(amount)","inc_date between '$open_bal_date' and '$currdate1'  && consignorid='$consignorid' "); 
			$otherexpense = $this->getvalfield($connection,"other_expense_entry","sum(amount)","exp_date between '$open_bal_date' and '$currdate1'  && consignorid='$consignorid' "); 
				$saleincome = $this->getvalfield($connection,"saleentry","sum(amount)","saledate between '$open_bal_date' and '$currdate1'  && consignorid='$consignorid' && payment_mode ='Cash'"); 
			// $truckexpense = $this->getvalfield($connection,"truck_expense as A left join m_userlogin as B on A.createdby = B.userid","sum(uchantiamount)","payment_type='cash' && paymentdate between '$open_bal_date' and '$currdate'  && branchid=1 && A.createdby in (select userid from m_userlogin where compid=$compid)"); 
			
												
						$openingbal = $cashopeningbal - $cementadv - $otherexpense + $otherincome + $saleincome;
...
function getcode1($connection,$tablename,$tablepkey,$cond)
{
	$num =  $this->getvalfield($connection,$tablename,"max($tablepkey)",$cond);
	//if($num == NULL)
	//$num = 0;
...
function getcode2($connection,$tablename,$tablepkey,$cond)
{
	$num =  $this->getvalfield($connection,$tablename,"max($tablepkey)",$cond);
	//if($num == NULL)
	//$num = 0;
