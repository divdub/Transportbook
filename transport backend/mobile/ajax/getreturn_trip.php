...
$getQuery = "SELECT * FROM trip_entry $crit && sessionconsignor_id=$consignorid LIMIT $initial_page, $limit";     
			...
			<?php 
				...
				//    $sql = mysqli_query($connection, "select * from dispatch_entry $crit order by dispatch_id  desc limit 10 ");
				   ...
				   while ($row = mysqli_fetch_array($result)) {
					...
					// $owner_name=$cmn->getvalfield($connection,"m_vehicle_owner","owner_name","consignor_id=$row[consignor_id]");
			...
			<div class="col-12 custom-col2">
				<div class="card p-3">
					<div class="row report-1 d-flex justify-content-between align-items-center">
						...
						<div class="col-4 px-3"><p>Trip No.</p>
							<span><?php echo $row['trip_no']; ?></span></div>
							<div class="col-4 px-3"><p>Loading Date</p><span><?php echo dateformatindia($row['loding_date']); ?></span></div>
							<div class="col-4 px-3"><p>Truck No.</p><span><?php echo $vehicle_no; ?></span></div>
							
						</div>
						<hr class="my-2">
						<div class="row report-1 d-flex justify-content-between align-items-center">
							<div class="col-4 px-3"><p>Consignor</p><span><?php echo $consignor_name; ?></span></div>
							<div class="col-4 px-3"><p>Wt (MT)</p><span><?php echo $row['qty_mt_day_trip']; ?></span></div>
							<div class="col-4 px-3"><p>Comp Rate</p><span><?php echo $row['rate']; ?></span></div>
						</div>
					</div>
				</div>	
				<?php } ?>
...
$getQuery = "SELECT COUNT(*) FROM trip_entry $crit && sessionconsignor_id=$consignorid";     
...
// alert(page);
    ...
    jQuery.ajax({
          type: 'POST',
          url: 'ajax/getreturn_trip.php',
          data: "page="+page+"&fromdate="+fromdate+"&todate="+todate+"&vehicle_id="+vehicle_id+"&consignor_id="+consignor_id,
          dataType: 'html',
