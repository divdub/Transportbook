...
if ($tag == "paid_report") {
        ...
        if ($truck_id != '') {
          ...
          $sql = mysqli_query($con, "SELECT * FROM dispatch_entry where 1=1 $crit and owner_id='$user_id' and is_complete='0'");
            }
            
            if($user_type =='2'){
                // echo "SELECT * FROM dispatch_entry where 1=1 $crit and driver_id='$user_id'"; die;
         $sql = mysqli_query($con, "SELECT * FROM dispatch_entry where 1=1 $crit and driver_id='$user_id' and is_complete='0'");
            }
            if($user_type =='3'){
                // echo "SELECT * FROM dispatch_entry where 1=1 $crit and pump_id='$user_id'"; die;
        $sql = mysqli_query($con, "SELECT * FROM dispatch_entry where 1=1 $crit and pump_id='$user_id' and is_complete='0'");
            }
            if($user_type =='4'){
                // echo "SELECT * FROM dispatch_entry where 1=1 $crit and pump_id='$user_id'"; die;
        $sql = mysqli_query($con, "SELECT * FROM dispatch_entry where 1=1 $crit and is_complete='0'");
            }
