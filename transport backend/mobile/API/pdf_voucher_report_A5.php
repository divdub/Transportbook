<?php

error_reporting(0);
include("config.php");
include('top_file.php');

// include('getval.php');

include("../../fpdf17/rotation.php");

  $cond=' ';
  
  	$comp_id = trim(addslashes($_GET['comp_id']));
	$session_id = trim(addslashes($_GET['sessionid']));

  
  if(isset($_GET['voucher_id']))
  {
    $voucher_no = trim(addslashes($_GET['voucher_id']));
  }
  else
  $voucher_no = '';
  
  if(isset($_GET['category_id']))
  {
    $category_id = trim(addslashes($_GET['category_id']));  
  }
  else
   $category_id = '';
  

  if(isset($_GET['catname']))
  {
    $catname = trim(addslashes($_GET['catname']));  
  }
  else
   $catname = '';
  

  
  $cond= " where 1=1";
  // if($from !='1970-01-01' && $todate !='1970-01-01') {
    
  //   $cond .=" and  B.tokendate BETWEEN '$from' and '$todate'";
    
  //   }
    if($voucher_no !='') {
    
    $cond .=" and voucher_id='$voucher_no'";
    }
      if($catname !='') {
    
    $cond .=" and catname='$catname'";
    }

    if($category_id !='') {
    
    $cond .=" and category_id='$category_id'";
    }
  

$c_logo =  getvalfield($con,"m_company","clogo","comp_id='$comp_id'");

function convert_number_to_words($number) {



    $hyphen      = '-';

    $conjunction = ' and ';

    $separator   = ', ';

    $negative    = 'negative ';

    $decimal     = ' point ';

    $dictionary  = array(

        0                   => 'zero',

        1                   => 'one',

        2                   => 'two',

        3                   => 'three',

        4                   => 'four',

        5                   => 'five',

        6                   => 'six',

        7                   => 'seven',

        8                   => 'eight',

        9                   => 'nine',

        10                  => 'ten',

        11                  => 'eleven',

        12                  => 'twelve',

        13                  => 'thirteen',

        14                  => 'fourteen',

        15                  => 'fifteen',

        16                  => 'sixteen',

        17                  => 'seventeen',

        18                  => 'eighteen',

        19                  => 'nineteen',

        20                  => 'twenty',

        30                  => 'thirty',

        40                  => 'fourty',

        50                  => 'fifty',

        60                  => 'sixty',

        70                  => 'seventy',

        80                  => 'eighty',

        90                  => 'ninety',

        100                 => 'hundred',

        1000                => 'thousand',

        1000000             => 'million',

        1000000000          => 'billion',

        1000000000000       => 'trillion',

        1000000000000000    => 'quadrillion',

        1000000000000000000 => 'quintillion'

    );



    if (!is_numeric($number)) {

        return false;

    }



    if (($number >= 0 && (int) $number < 0) || (int) $number < 0 - PHP_INT_MAX) {

        // overflow

        trigger_error(

            'convert_number_to_words only accepts numbers between -' . PHP_INT_MAX . ' and ' . PHP_INT_MAX,

            E_USER_WARNING

        );

        return false;

    }



    if ($number < 0) {

        return $negative . convert_number_to_words(abs($number));

    }



    $string = $fraction = null;



    if (strpos($number, '.') !== false) {

        list($number, $fraction) = explode('.', $number);

    }



    switch (true) {

        case $number < 21:

            $string = $dictionary[$number];

            break;

        case $number < 100:

            $tens   = ((int) ($number / 10)) * 10;

            $units  = $number % 10;

            $string = $dictionary[$tens];

            if ($units) {

                $string .= $hyphen . $dictionary[$units];

            }

            break;

        case $number < 1000:

            $hundreds  = $number / 100;

            $remainder = $number % 100;

            $string = $dictionary[$hundreds] . ' ' . $dictionary[100];

            if ($remainder) {

                $string .= $conjunction . convert_number_to_words($remainder);

            }

            break;

        default:

            $baseUnit = pow(1000, floor(log($number, 1000)));

            $numBaseUnits = (int) ($number / $baseUnit);

            $remainder = $number % $baseUnit;

            $string = convert_number_to_words($numBaseUnits) . ' ' . $dictionary[$baseUnit];

            if ($remainder) {

                $string .= $remainder < 100 ? $conjunction : $separator;

                $string .= convert_number_to_words($remainder);

            }

            break;

    }



    if (null !== $fraction && is_numeric($fraction)) {

        $string .= $decimal;

        $words = array();

        foreach (str_split((string) $fraction) as $number) {

            $words[] = $dictionary[$number];

        }

        $string .= implode(' ', $words);

    }



    return $string;

}
 function getinwordsbyindia($number)
{
   $no = round($number);
   $point = round($number - $no, 2) * 100;
   $hundred = null;
   $digits_1 = strlen($no);
   $i = 0;
   $str = array();
   $words = array('0' => '', '1' => 'one', '2' => 'two',
    '3' => 'three', '4' => 'four', '5' => 'five', '6' => 'six',
    '7' => 'seven', '8' => 'eight', '9' => 'nine',
    '10' => 'ten', '11' => 'eleven', '12' => 'twelve',
    '13' => 'thirteen', '14' => 'fourteen',
    '15' => 'fifteen', '16' => 'sixteen', '17' => 'seventeen',
    '18' => 'eighteen', '19' =>'nineteen', '20' => 'twenty',
    '30' => 'thirty', '40' => 'forty', '50' => 'fifty',
    '60' => 'sixty', '70' => 'seventy',
    '80' => 'eighty', '90' => 'ninety');
   $digits = array('', 'hundred', 'thousand', 'lakh', 'crore');
   while ($i < $digits_1) {
     $divider = ($i == 2) ? 10 : 100;
     $number = floor($no % $divider);
     $no = floor($no / $divider);
     $i += ($divider == 10) ? 1 : 2;
     if ($number) {
        $plural = (($counter = count($str)) && $number > 9) ? '' : null;
        $hundred = ($counter == 1 && $str[0]) ? 'and ' : null;
        $str [] = ($number < 21) ? $words[$number] .
            " " . $digits[$counter] . $plural . " " . $hundred
            :
            $words[floor($number / 10) * 10]
            . " " . $words[$number % 10] . " "
            . $digits[$counter] . $plural . " " . $hundred;
     } else $str[] = null;
  }
  $str = array_reverse($str);
  $result = implode('', $str);
  $points = ($point) ?
    "." . $words[$point / 10] . " " . 
          $words[$point = $point % 10] :'';
		  
		
		  
if($points !='' && $points !='0')
{
 $words=  "$result Rupees $points  Paise";
}
else
{
	$words=  "$result Rupees  ";
}
   
   return $words;
}

class PDF extends PDF_Rotate
{
    
    var $widths;
  
  var $aligns;
    function Header()
    {
         global $adv_date,$bal_amt,$receive_date;
        /* Put the watermark */
        
        if($bal_amt=='0'){
        $this->SetFont('Arial','B',80);
        $this->SetTextColor(255,192,203);
        $this->RotatedText(100,150,'PAID',45);
        $this->SetFont('Arial','B',18);
        $this->RotatedText(110,150,$receive_date,45);
        }
       if($receive_date!='' && $bal_amt > 0){
            $this->SetFont('Arial','B',80);
            $this->SetTextColor(255,192,203);
            $this->RotatedText(100,150,'Pending',45);
            $this->SetFont('Arial','B',18);
            //$this->RotatedText(110,150,$receive_date,45); 
        }
        $this->Rect(2,2,206,145,'D');
    }
    
    function Footer()

    { 

     global $deduct,$ac_number,$ifsc_code,$branch_name,$bank_name ;

     $this->SetY(-30);

}

     }
    

    function RotatedText($x, $y, $txt, $angle)
    {
        /* Text rotated around its origin */
        $this->Rotate($angle,$x,$y);
        $this->Text($x,$y,$txt);
        $this->Rotate(0);
    }
    
    function SetWidths($w)

{

    //Set the array of column widths

    $this->widths=$w;

}



function SetAligns($a)

{

    //Set the array of column alignments

    $this->aligns=$a;

}
function Row($data)

{

    //Calculate the height of the row

    $nb=0;

    for($i=0;$i<count($data);$i++)

        $nb=max($nb,$this->NbLines($this->widths[$i],$data[$i]));

    $h=5*$nb;

    //Issue a page break first if needed

    $this->CheckPageBreak($h);

    //Draw the cells of the row

    for($i=0;$i<count($data);$i++)

    {

        $w=$this->widths[$i];

        $a=isset($this->aligns[$i]) ? $this->aligns[$i] : 'L';

        //Save the current position

        $x=$this->GetX();

        $y=$this->GetY();

        //Draw the border

        $this->Rect($x,$y,$w,$h);

        //Print the text

        $this->MultiCell($w,5,$data[$i],0,$a);

        //Put the position to the right of the cell

        $this->SetXY($x+$w,$y);

    }

    //Go to the next line

    $this->Ln($h);

}

function CheckPageBreak($h)

{

    //If the height h would cause an overflow, add a new page immediately

    if($this->GetY()+$h>$this->PageBreakTrigger)

        $this->AddPage($this->CurOrientation);

}
function NbLines($w,$txt)

{

    //Computes the number of lines a MultiCell of width w will take

    $cw=&$this->CurrentFont['cw'];

    if($w==0)

        $w=$this->w-$this->rMargin-$this->x;

    $wmax=($w-2*$this->cMargin)*1000/$this->FontSize;

    $s=str_replace("\r",'',$txt);

    $nb=strlen($s);

    if($nb>0 and $s[$nb-1]=="\n")

        $nb--;

    $sep=-1;

    $i=0;

    $j=0;

    $l=0;

    $nl=1;

    while($i<$nb)

    {

        $c=$s[$i];

        if($c=="\n")

        {

            $i++;

            $sep=-1;

            $j=$i;

            $l=0;

            $nl++;

            continue;

        }

        if($c==' ')

            $sep=$i;

        $l+=$cw[$c];

        if($l>$wmax)

        {

            if($sep==-1)

            {

                if($i==$j)

                    $i++;

            }

            else

                $i=$sep+1;

            $sep=-1;

            $j=$i;

            $l=0;

            $nl++;

        }

        else

            $i++;

    }

    return $nl;

}

    


function GenerateWord()

{

    //Get a random word

    $nb=rand(3,10);

    $w='';

    for($i=1;$i<=$nb;$i++)

        $w.=chr(rand(ord('a'),ord('z')));

    return $w;

}



function GenerateSentence()

{

    //Get a random sentence

    $nb=rand(1,10);

    $s='';

    for($i=1;$i<=$nb;$i++)

        $s.=GenerateWord().' ';

    return substr($s,0,-1);

}

    
 
$pdf=new PDF('L','mm','A5');
$pdf->AddPage();
/*output the result*/
$pdf->SetY(5);

 $pdf->SetX(20);
  
 
                                       
                                          
                                            $company_owner = "SELECT * FROM `m_company` where comp_id='$comp_id' ";
                                             

                                    $cmp_owm = mysqli_query($con,$company_owner);
                               while($onwer = mysqli_fetch_assoc($cmp_owm)){
    
                                    //   $pdf->Image('../../upload/logo/'.$c_logo,10,3,13);
                                                    
                                        $pdf->SetX(70);
                                         $pdf->SetFont('Arial','',24);
                                        $pdf->Cell(150,8,$onwer['cname'],2,0,'L');
                                        
                                         $pdf->SetX(130);
                                         $pdf->SetFont('Arial','',24);
                                        $pdf->Cell(150,8,'',2,0,'L');
                                        
                                        $pdf->Ln();

                                        $pdf->SetX(50);
                                         $pdf->SetFont('Arial','',10);
                                        $pdf->Cell(270,6,$onwer['caddress'],2,0,'L');
                                        $pdf->Ln();

                                            
                                         $pdf->SetFont('Arial','',10);
                                       $pdf->Cell(350,5,'',2,0,'L');
                                        $pdf->SetFont('Arial','',10);
                                       $pdf->Cell(125,-20,'Contact : '.$onwer['mobileno1'].",".$onwer['mobileno2'],2,0,'R');
                                         
                                               

                                        $pdf->SetFont('Arial','',10);
                                       $pdf->Cell(150,5,'',2,0,'L');
                                        $pdf->SetFont('Arial','',10);
                                        $pdf->Cell(120,-10,'GST No. : '.$onwer['gst_no'],2,0,'R');
                                        $pdf->Ln();

                                         
                                                                               

                                        $pdf->SetFont('Arial','',10);

                                       $pdf->Cell(150,10,'',2,0,'R');

                                        $pdf->SetFont('Arial','',10);
                                        $pdf->Cell(125,8,'PAN No. : '.$onwer['pan_no'],2,0,'R');
                                        $pdf->Ln();


                                         $pdf->SetFont('Arial','',10);

                                       $pdf->Cell(150,10,'',2,0,'R');

                                        $pdf->SetFont('Arial','',10);
                                        $pdf->Cell(125,2,'GST No. : '.$onwer['gst_no'],2,0,'R');
                                        $pdf->Ln();

                                           

                                           $lh = 100;
                                            $lw= $lh;

                                            $pdf->Line(2, 18, 208,18);
                                             $pdf->Ln(0);

                                              $pdf->SetFont('Arial','',10);

                                       $pdf->Cell(30,10,'',2,0,'L');
                                         $pdf->SetFont('Arial','B',12);

}
               
                            
                   $invoicquery = "SELECT * from payment  $cond  && comp_id=$comp_id && session_id=$session_id";
     
                                            $invres = mysqli_query($con,$invoicquery);
                                        
                                      
                                    $incrow = mysqli_fetch_array($invres);
 

                                    $voucher_date=$incrow['voucher_date']; 
                                   
                                    $payee_name=$incrow['payee_name']; 
                                    
                                    $category_id=$incrow['category_id']; 
                                       
                                    $ifsc_code=$incrow['ifsc_code']; 
                                    $acc_no=$incrow['acc_no']; 
                                     $voucher=$incrow['voucher_id'];
                                     
                                    if($category_id==1){
                                    $cname="Agent";
                                
                //  $voucher_no=$incrow['voucher_id'];
    // $voucher="AG-".$voucher_no;
$agent_id= getvalfield($con,"dispatch_entry","agent_id","dispatch_id='$incrow[dispatch_id]'");
$vname= getvalfield($con,"m_agent","agent_name","agent_id='$agent_id'");
                                    }
                 if($category_id==2){
                                $cname="Consignee";
    // $voucher_no=$incrow['voucher_no'];
    // $voucher="CO-".$voucher_no;
$consignee_id= getvalfield($con,"dispatch_entry","consignee_id","dispatch_id='$incrow[dispatch_id]'");
$vname= getvalfield($con,"m_consignee","consignee_name","consignee_id='$consignee_id'");
                                    }
                                     if($category_id==4){
                                 $cname="Truck Owner";
    // $voucher_no=$incrow['voucher_no'];
    // $voucher="TO-".$voucher_no;
$owner_id= getvalfield($con,"dispatch_entry","owner_id","dispatch_id='$incrow[dispatch_id]'");
$vname= getvalfield($con,"m_vehicle_owner","owner_name","owner_id='$owner_id'");
                                    }
      $remark=$incrow['remark']; 
    //   $pdf->SetX(50);
        
                                      $pdf->SetY(18);

                                        $pdf->SetFont('Arial','B',9);
                                        $pdf->SetX(2);
                                        $pdf->Cell(51,8,'Category',1,0,'C');
                                         
                                        $pdf->SetFont('Arial','',9); 
                                      $pdf->Cell(51,8,$cname ,1,0,'C');
                        //$pdf->Cell(125,6,'Category: '.$cname ,1,0,'L');
                                         
                                               

                                    //     $pdf->SetFont('Arial','',10);

                                    //   $pdf->Cell(45,10,'',2,0,'L');

                                        // $pdf->SetFont('Arial','',10);
                                        
                                        $pdf->SetY(18);

                                        $pdf->SetFont('Arial','B',9);
                                        $pdf->SetX(104);
                                        $pdf->Cell(52,8,'Voucher No.:',1,0,'C');
                                        $pdf->SetFont('Arial','',9);
                                        $pdf->Cell(52,8,$voucher ,1,0,'C');
                                        
                                        $pdf->Ln(8);

                                        $pdf->SetY(26);

                                        $pdf->SetFont('Arial','B',9);
                                        $pdf->SetX(2);
                                        $pdf->Cell(51,8,'Voucher Name :',1,0,'C');
                                         
                                        $pdf->SetFont('Arial','',9);
                                      $pdf->Cell(51,8,$vname ,1,0,'C');
                                      // $pdf->Cell(125,10,'Voucher Name : '.$vname ,1,0,'L');
                                      //$pdf->Ln(15);
                                      $pdf->SetY(26);
      
                                      $pdf->SetFont('Arial','B',9);
                                      $pdf->SetX(104);
                                      $pdf->Cell(52,8,'Voucher Date:',1,0,'C');
                                      $pdf->SetFont('Arial','',9);
                                      $pdf->Cell(52,8,date('d-m-Y',strtotime($voucher_date)) ,1,0,'C');
                                       // $pdf->Cell(5,10,'',2,0,'L');
                                         $pdf->SetFont('Arial','',9);
                                      // $pdf->Cell(80,10,'Voucher Date : '.date('d-m-Y',strtotime($voucher_date)) ,2,1,'L');
                                       
                                             

                                    //     $pdf->SetFont('Arial','',10);

                                    //   $pdf->Cell(45,10,'',2,0,'L');

                                    //     $pdf->SetFont('Arial','',10);
                                    //     $pdf->Cell(95,8,'Location: Raipur',2,1,'L');
                                         //$pdf->Ln(10);
                                         $pdf->SetY(34);

                                        $pdf->SetFont('Arial','B',9);
                                        $pdf->SetX(2);
                                        $pdf->Cell(51,8,'Paid To :',1,0,'C');
                                         
                                        $pdf->SetFont('Arial','',9);
                                      $pdf->Cell(51,8,$payee_name ,1,0,'C');
                                         $pdf->SetFont('Arial','B',9);
                                          $pdf->Cell(5,10,'',2,0,'L');
                                       // $pdf->Cell(125,8,'Paid To : '.$payee_name ,2,0,'L');
                                          // $toplace = $cmn->getvalfield($connection,"m_place","placename","placeid='$destinationid'"); 
                                          $pdf->SetY(34);
      
                                          $pdf->SetFont('Arial','B',9);
                                          $pdf->SetX(104);
                                          $pdf->Cell(52,8,'Remark',1,0,'C');
                                          $pdf->SetFont('Arial','',9);
                                          $pdf->Cell(52,8,$remark ,1,0,'C');
                                         
                                          $pdf->SetY(42);

                                          $pdf->SetFont('Arial','B',9);
                                          $pdf->SetX(2);
                                          $pdf->Cell(51,8,'Account No. :',1,0,'C');
                                          $pdf->SetFont('Arial','',9);
                                        $pdf->Cell(51,8,$acc_no ,1,0,'C');
                                        
                                          // $toplace = $cmn->getvalfield($connection,"m_place","placename","placeid='$destinationid'"); 
                                         $pdf->SetY(42);
      
                                          $pdf->SetFont('Arial','B',9);
                                          $pdf->SetX(104);
                                          $pdf->Cell(52,8,'IFSC Code',1,0,'C');
                                          $pdf->SetFont('Arial','',9);
                                          $pdf->Cell(52,8,$ifsc_code ,1,0,'C');
                                          
                                          //$pdf->Ln(30);
                                          //$pdf->SetX(200);
                                          //$pdf->SetY(20);
                                        $pdf->SetFont('Arial','B',10);
                                         $pdf->SetX(50);
                                         $pdf->SetY(52);
                                          $pdf->Cell(0,5,'Voucher (Bill Details) ',0,0,'C');
                                        
                                        
                                    
                                      
                       $pdf->Ln(4);
                                        
                                        
                                          $pdf->SetFont('Arial','',10);

                                       $pdf->Cell(5,10,'',2,0,'L');

                                      
                                        
                                        $pdf->Ln(3);
                                        $pdf->SetX(2);

                                            $pdf->SetFont('Arial','B',8);
                                                    $pdf->Cell(6,8,'S.N',1,0,'C');

                                                     

                                                     $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(15,8,'DI/LR No',1,0,'L');
                                                     
                                                      
                                                     $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(16,8,'Bilty Date',1,0,'R');
                                                     


                                                     $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(21,8,'Truck No',1,0,'R');

                                                     $pdf->SetFont('Arial','B',8);
                                                   //  $pdf->Cell(30,8,'Destination',1,0,'L');
                                                      $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(22,8,'Destination',1,0,'L');
                                                     
                                                    //  $pdf->SetFont('Arial','B',8);
                                                    //  $pdf->Cell(12,8,'Wt.(MT)',1,0,'R');
                                                     $pdf->SetFont('Arial','B',7);
                                                     $pdf->Cell(10,8,'Rec.wt',1,0,'R');
                                                     $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(10,8,'Co.Rt',1,0,'R');

                                                    //  $pdf->SetFont('Arial','B',7);
                                                    //  $pdf->Cell(13,8,'Commi.',1,0,'R');

                                                     $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(10,8,'O.Rt',1,0,'L');
                                                     
                                                      $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(13,8,'Frt Amt',1,0,'R');

                                                    $pdf->SetFont('Arial','B',7);
                                                     $pdf->Cell(9,8,"Comm",1,0,'R');

                                                     $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(8,8,'Bank',1,0,'R');

                                                     $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(9,8,'Shrt',1,0,'R');


                                                    // $pdf->SetFont('Arial','B',8);
                                                    //  $pdf->Cell(8,8,'TDS',1,0,'R');

                                                    $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(8,8,'TDS',1,0,'R');

                                                     $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(10,8,'Di Adv',1,0,'C');
                                                     
                                                      $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(13,8,'Cash Adv',1,0,'C');
                                                     
                                                      $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(11,8,'GPS ',1,0,'C');
                                                    //       $pdf->SetFont('Arial','B',8);
                                                    //  $pdf->Cell(18,8,'Consignor',1,0,'C');
                                                    //      $pdf->SetFont('Arial','B',8);
                                                    //  $pdf->Cell(18,8,'Consignee',1,0,'C');
                                                    
                                                     $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(15,8,' Net Amt',1,1,'R');
                                                    


     
                    // echo  "SELECT * from payment  $cond ";    
                      $sn=1;  
                   $sel = "SELECT * from payment  $cond &&  comp_id=$comp_id && session_id=$session_id";
        $res = mysqli_query($con,$sel);
                                    while($row = mysqli_fetch_array($res))
                                    {
           
          
 $bilty_date=  getvalfield($con,"dispatch_entry","bilty_date","dispatch_id='$row[dispatch_id]'");   
$di_no=  getvalfield($con,"dispatch_entry","di_no","dispatch_id='$row[dispatch_id]'");
$vehicle_id = getvalfield($con,"dispatch_entry","vehicle_id","dispatch_id='$row[dispatch_id]'");
$destination_id = getvalfield($con,"dispatch_entry","destination_id","dispatch_id='$row[dispatch_id]'");
$comp_rate = getvalfield($con,"dispatch_entry","comp_rate","dispatch_id='$row[dispatch_id]'");

$wt_mt = getvalfield($con,"dispatch_entry","wt_mt","dispatch_id='$row[dispatch_id]'");
$rec_wt = getvalfield($con,"dispatch_entry","rec_wt","dispatch_id='$row[dispatch_id]'");
$destination = getvalfield($con,"m_place","place_name","place_id='$destination_id'");
$vehicle_no =  getvalfield($con,"m_vehicle","vehicle_no","vehicle_id='$vehicle_id'");

 
 $freight_amt = $row['freight_amt'];
  $freight_rate=$row['freight_rate'];
 $sort_wt= $wt_mt - $rec_wt;
 $bilty_commision=$row['bilty_commision'];
 $sortamt=$row['sortamt'];
  $tds=$row['tds'];
 $tds_amt=$row['tds_amt']; 
     $diesel_adv_amt=$row['diesel_adv_amt'];   
     $cash_adv=$row['cash_adv'];    
     $other_cash_adv=$row['other_cash_adv'];   
  $consignor_cash_adv=$row['consignor_cash_adv'];    
     $consignee_cash_adv=$row['consignee_cash_adv'];
    $amt_paid_to=$row['amt_paid_to'];
 $bank_charge=$row['bank_charge'];

      $pdf->SetFont('Arial','',8);
      $pdf->SetX(2);
                                                    $pdf->Cell(6,8, $sn++,1,0,'L');

                                                     

                                                     $pdf->SetFont('Arial','',8);
                                                     $pdf->Cell(15,8,$di_no,1,0,'L');
                                                     
                                                    
                                                     $pdf->SetFont('Arial','',8);
                                                     $pdf->Cell(16,8,date('d-m-Y',strtotime($bilty_date)),1,0,'R');
                                                     


                                                     $pdf->SetFont('Arial','',8);
                                                     $pdf->Cell(21,8,$vehicle_no,1,0,'R');

                                                     $pdf->SetFont('Arial','',8);
                                                   //  $pdf->Cell(30,8,$toplace,1,0,'L');
                                                     
                                                      $pdf->SetFont('Arial','',8);
                                                     $pdf->Cell(22,8,$destination,1,0,'C');
                                                     
                                                    //  $pdf->SetFont('Arial','',8);
                                                    //  $pdf->Cell(12,8, $wt_mt,1,0,'R');

                                                     $pdf->SetFont('Arial','',8);
                                                     $pdf->Cell(10,8,$rec_wt,1,0,'R');

                                                    //  $pdf->SetFont('Arial','',8);
                                                    //  $pdf->Cell(13,8,$rate_mt,1,0,'R');


                                                    $pdf->SetFont('Arial','',8);
                                                     $pdf->Cell(10,8,$comp_rate,1,0,'R');

                                                     $pdf->SetFont('Arial','',8);
                                                    $pdf->Cell(10,8,round($freight_rate),1,0,'R');
                                                    
                                                     $pdf->Cell(13,8,round($freight_amt),1,0,'R');
                                                     $pdf->SetFont('Arial','',8);

                                                     $pdf->SetFont('Arial','',8);
                                                     $pdf->Cell(9,8,round($bilty_commision),1,0,'R');
                                                     $pdf->SetFont('Arial','',8);
                                                     $pdf->Cell(8,8,$bank_charge,1,0,'R');

                                                     $pdf->SetFont('Arial','',8);
                                                     $pdf->Cell(9,8,$sortamt,1,0,'R');


                                                    // $pdf->SetFont('Arial','',8);
                                                    //  $pdf->Cell(8,8,$tds.'%',1,0,'R');

                                                    $pdf->SetFont('Arial','',8);
                                                     $pdf->Cell(8,8,round($tds_amt),1,0,'R');

                                                     $pdf->SetFont('Arial','',8);
                                                     $pdf->Cell(10,8,round($diesel_adv_amt),1,0,'R');
                                                      $pdf->SetFont('Arial','',8);
                                                     $pdf->Cell(13,8,round($cash_adv),1,0,'R');
                                                      $pdf->SetFont('Arial','',8);
                                                $pdf->Cell(11,8,round($other_cash_adv),1,0,'R');
                                                    //   $pdf->SetFont('Arial','',8);
                                                    //  $pdf->Cell(18,8,round($consignor_cash_adv),1,0,'R');
                                                    //   $pdf->SetFont('Arial','',8);
                                                    //  $pdf->Cell(18,8,round($consignee_cash_adv),1,0,'R');
                                                     $pdf->SetFont('Arial','',8);
                                                    
                                                    $pdf->Cell(15,8,number_format(round($amt_paid_to),2),1,0,'R');

      
                                                 
                                                 

                                                      $pdf->Ln();
                                                    $total_wt_mt+=$wt_mt;
                                                $total_recweight+=$rec_wt;
                                                $total_final_rate+=$freight_rate;
                                                    $total_rate_mt+=$rate_mt;
                                                // $total_trip_commission+=$bilty_commision;
                                                $total_commision+=$bilty_commision;
                                                $total_freight+=$freight_amt;
                                                $total_deduct+=$sortamt;    
                                                $total_tds_amount+=$tds_amt;
                                                $total_adv +=$cash_adv;
                                                $total_desial_adv += $diesel_adv_amt;
                                                $total_netamout +=$netamount;
                                                $toal_advconsi +=$consignor_cash_adv;
                                                $great_total +=$amt_paid_to;
                                                $total_tds+=$tds_amt;
                                             $total_bank_charge+=$bank_charge;


                                           }  
                                                     
                                        $pdf->SetX(2);
                                                     $pdf->SetFont('Arial','B',9);
                                                     $pdf->Cell(80,10,'Total',1,0,'C');

                                                     
                                                     
                                                    //  $pdf->SetFont('Arial','B',8);
                                                    //  $pdf->Cell(12,12, $total_wt_mt,1,0,'R');
                                                     $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(10,10,$total_recweight,1,0,'R');
                                                    //  $pdf->SetFont('Arial','B',8);
                                                    //  $pdf->Cell(13,10,$total_rate_mt,1,0,'R');

                                                     $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(10,10,'',1,0,'R');

                                                    $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(10,10,'',1,0,'R');
                                                     
                                                      $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(13,10,round($total_freight),1,0,'R');
                                                     

                                                 $pdf->SetFont('Arial','B',8);
                                             $pdf->Cell(9,10,round($total_commision),1,0,'R');
                                                        $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(8,10,$total_bank_charge,1,0,'R');
                                                     $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(9,10,$total_deduct,1,0,'R');


                                                    $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(8,10,'',1,0,'R');

                                                    // $pdf->SetFont('Arial','B',8);
                                                    //  $pdf->Cell(8,10,'',1,0,'R');
                                                     

                                                     $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(10,10,round($total_desial_adv),1,0,'R');
                                                        $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(13,10,round($total_adv),1,0,'R');
                                                   
                                                       $pdf->SetFont('Arial','B',8);
                                                      $pdf->Cell(11,10,'',1,0,'R');
                                                        $pdf->SetFont('Arial','B',8);
                                                        
                                                   //  $pdf->Cell(24,10,'',1,0,'R');
                                                    
                                                     $pdf->SetFont('Arial','B',8);
                                                     $pdf->Cell(15,10,number_format(round($great_total),2),1,0,'R');



                                                      $pdf->Ln();
                                                     $pdf->SetX(2);
                                                      $pdf->SetFont('Arial','B',10);
                                                     $pdf->Cell(140,10,'Grant Total Amount',1,0,'C');

                                                     
                                                     $pdf->Cell(65,10,number_format(round($great_total),2),1,1,'R');
                                                     $pdf->Cell(25,10,'In Words :',2,0,'C');

   $pdf->Cell(155,10,UCWORDS(getinwordsbyindia($great_total)),2,0,'L');

                                                  
                                                    
                                                      

                                               


                                                  
                                                     
                                                      

                                               

 $pdf->Output();
?>
