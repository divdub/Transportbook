...
if ($tag == "paid_report") {
        ...
        if ($truck_id != '') {
          ...
          $sql = mysqli_query($con, "SELECT * FROM dispatch_entry where 1=1 $crit and owner_id='$user_id' and is_complete='1'");
            }
            
            if($user_type =='2'){
                // echo "SELECT * FROM dispatch_entry where 1=1 $crit and driver_id='$user_id'"; die;
         $sql = mysqli_query($con, "SELECT * FROM dispatch_entry where 1=1 $crit and driver_id='$user_id' and is_complete='1'");
            }
            if($user_type =='3'){
                // echo "SELECT * FROM dispatch_entry where 1=1 $crit and pump_id='$user_id'"; die;
        $sql = mysqli_query($con, "SELECT * FROM dispatch_entry where 1=1 $crit and pump_id='$user_id' and is_complete='1'");
            }
            if($user_type =='4'){
                // echo "SELECT * FROM dispatch_entry where 1=1 $crit and pump_id='$user_id'"; die;
        $sql = mysqli_query($con, "SELECT * FROM dispatch_entry where 1=1 $crit and is_complete='1'");
            }
    ...
    if ($count > 0) {
        ...
        while ($row = mysqli_fetch_assoc($sql)) {
            ...
            $consignee_name = getvalfield($con, "m_consignee", "consignee_name", "consignee_id='$consignee_id'");
             ...
             $voucher_id = getvalfield($con,"payment","voucher_id","dispatch_id='$row[dispatch_id]' && category_id='4'");
			 $payment= getvalfield($con,"payment","freight_rate","dispatch_id='$row[dispatch_id]' && category_id='4'");
			 $payee_name = getvalfield($con,"payment","payee_name","voucher_id='$voucher_id' && comp_id=$comp_id && session_id=$session_id");
			 $paydate1 = getvalfield($con,"payment_receive","receive_date","voucher_no='$voucher_id' && comp_id=$comp_id && session_id=$session_id");
			 $paydate = date('d-m-Y', strtotime($paydate1));			
             $bilty_date = date('d-m-Y', strtotime($row['bilty_date']));
             $famt=  $row['wt_mt'] * $payment;
            ...
            $row['famt'] = $famt;
            $row['paydate'] = $paydate;
            $row['voucher_no'] = $voucher_id;
            $row['paid_to'] = $payee_name;
             array_push($data, $row);
             $success = true;
           $msg = "Record Found";
        }
