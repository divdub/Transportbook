...
<div class="content-body" >
						...
						<label for="form-check-label">Truck No.</label>
						 ...
						 <select name="vehicle_id" id="vehicle_id" class='form-select mySelect' style="width:100%;" aria-label="Default select example" onChange="getSearch();">
                                          ...
                                          <script>document.getElementById('vehicle_id').value = '<?php echo $vehicle_id; ?>';</script>
                                                ...
                                                <div class="col-md-6 col-6">
                    <div class="mb-3">
						<label for="form-check-label">Documents</label>
						
						 <select name="doccat_id" id="doccat_id" class='form-select mySelect' style="width:100%;" aria-label="Default select example" onChange="getSearch();">
                                             <option value="">      Select  </option>
                                             <?php		$sql = mysqli_query($connection,"Select * from  m_doc_cat  order by doccat_id");
                                                while($row= mysqli_fetch_array($sql)) { ?>
                                             <option value="<?php echo $row['doccat_id']; ?>"><?php echo $row['catname']; ?></option>
                                             <?php } ?>
                                          </select>
                                          <script>document.getElementById('doccat_id').value = '<?php echo $owner_id; ?>';</script>
    ...
    jQuery.ajax({
          ...
          data: "fromdate="+fromdate+"&todate="+todate+"&vehicle_id="+vehicle_id+"&doccat_id="+doccat_id,
          dataType: 'html',
