...
$pdf->SetX('2');
$pdf->Cell(293, 8, 'Incentive GST Payment Details', 1, 0, 'C', 'F');
$pdf->Ln();
...
$pdf->Cell(30, 6, 'Ref. No', 1, 0, 'L', 1);
// 	$pdf->Cell(24,6,'GR/TR NO',1,0,'L',1);
$pdf->Cell(30, 6, 'Date', 1, 0, 'C', 1);
$pdf->Cell(50, 6, 'GST Amount', 1, 0, 'C', 1);
$pdf->Cell(30, 6, 'Receive Date', 1, 0, 'C', 1);
...
$pdf->Cell(40, 6, 'Received Amount', 1, 0, 'C', 1);
$pdf->Cell(60, 6, 'Remark', 1, 1, 'C', 1);
...
$sn = 1;
//  echo "Select * from  manualinv $crit && gst_pay=1 && type='incentive' && consignorid=$consignorid && session_id=$session_id order by $tblpkey desc ";die;
 // 	echo "Select * from  invoicebilty $crit && is_pay=1 && consignorid=$consignorid && sessionid=$session_id order by minvid desc " ;
   $sql = mysqli_query($connection, "Select * from manualinv $crit && gst_pay=1 && type='incentive' && consignorid=$consignorid && session_id=$session_id order by minvid desc ");
while ($row = mysqli_fetch_array($sql)) {
    ...
    $pdf->Cell(30, 8, $row['ref_no'], 1, 0, 'R');
    // $pdf->Cell(24,8, $gr_no, 1,0, 'R');

    $pdf->Cell(30, 8, date('d-m-Y', strtotime($row['receive_date'])), 1, 0, 'R');
    $pdf->Cell(50, 8, $row['gst_amt'], 1, 0, 'C');
    $pdf->Cell(30, 8, dateformatindia($receive_gstdate), 1, 0, 'R');
    // $pdf->Cell(28,8, $deliverat, 1,0, 'R');
    //      $pdf->Cell(13,8, $slip_no,1,0,'L');
    //  $pdf->Cell(15,8, $dbillno, 1,0, 'R');

    $pdf->Cell(40, 8, $row['received_gstamt'], 1, 0, 'C');

    $pdf->Cell(60, 8, $row['gstremark'], 1, 0, 'C');

    $pdf->Ln();


    $treceived_gstamt += $row['received_gstamt'];
    $tgst_amt += $row['gst_amt'];
...
$pdf->SetFont('Arial', 'B', 9);
$pdf->Cell(68, 12, 'Total', 1, 0, 'C', 'F');



$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(50, 12, $tgst_amt, 1, 0, 'R', 'F');
$pdf->Cell(30, 12, '', 1, 0, 'R', 'F');
$pdf->Cell(40, 12, $treceived_gstamt, 1, 0, 'R', 'F');
...
$pdf->SetX(10);
$pdf->Cell(45, 8, 'Total GST Amount  :', 2, 0, 'L');


$pdf->SetX(55);
$pdf->Cell(90, 8, ucwords(getinwordsbyindia($tgst_amt)) . " Rupees Only", 2, 1, 'L');
$pdf->SetX(10);

$pdf->Cell(45, 8, 'Total Receive Amount : ', 2, 0, 'L');

$pdf->SetX(55);
$pdf->Cell(90, 8, ucwords(getinwordsbyindia($treceived_gstamt)) . " Rupees Only", 2, 1, 'L');
$pdf->SetX(10);
$pdf->Cell(45, 8, 'Balance   :', 2, 0, 'L');

$pdf->SetFont('Arial', 'B', 10);
$pdf->SetX(55);
$pdf->Cell(90, 8, ucwords(getinwordsbyindia($tgst_amt - $treceived_gstamt)) . " Rupees Only", 2, 1, 'L');
